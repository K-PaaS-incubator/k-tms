...
typedef struct st_print_event_info
{
  ...
  */
  // note: have the last catalog here ??
  char db[FN_REFLEN+1]; // note: make this a LEX_STRING when thd->db is
  bool flags2_inited;
  uint32 flags2;
...
public:
  ...
  /*
    ...
    note: this may become false if Dmitri pushes his new LOAD DATA INFILE in
    5.0 only (not in 4.x).
  */
  uint32 db_len;
  uint32 fname_len;
