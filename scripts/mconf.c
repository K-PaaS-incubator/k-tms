...
"Overview\n"
"--------\n"
"Some features may be built directly into TESS.  Some features\n"
...
load_config_help[] =
	"\n"
	"For various reasons, one may wish to keep several different TESS\n"
	"configurations available on a single machine.\n"
	"\n"
	"If you have saved a previous configuration in a file other than the\n"
	"TESS's default, entering the name of the file here will allow you\n"
...
save_config_help[] =
	"\n"
	"For various reasons, one may wish to keep different TESS\n"
	"configurations available on a single machine.\n"
	"\n"
...
static struct dialog_list_item *items[16384]; /* note: This ought to be dynamic. */
static int item_no;
...
int main(int ac, char **av)
{
	...
	sym_calc_value(sym);
	snprintf(menu_backtitle, 128, "TESS v%s Configuration", sym_get_string_value(sym));
	...
	string = getenv("__ARCH");

	if (string)
	{
		if (strncmp(string, "x86", 7) == 0) {
			sym = sym_lookup("CONFIG_ARCH_X8632", 0);
			sym_calc_value(sym);
			sym_set_tristate_value(sym, yes);
			sym_is_changable(sym);
		}
		else if (strncmp(string, "x86_64", 7) == 0) {
			sym = sym_lookup("CONFIG_ARCH_X8664", 0);
			sym_calc_value(sym);
			sym_set_tristate_value(sym, yes);
			sym_is_changable(sym);
		}
	...
	do {
		stat = dialog_yesno(NULL,
				    "Do you wish to save your new TESS configuration?", 5, 60);
	} while (stat < 0);
	end_dialog();
	...
	if (stat == 0) {
		conf_write(NULL);
		printf("\n"
			"*** End of TESS configuration.\n");
	} else
		printf("\n\nYour TESS configuration changes were NOT saved.\n\n");
