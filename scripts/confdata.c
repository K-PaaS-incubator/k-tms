...
int conf_write(const char *name)
{
	...
	if (!name) {
		...
		if (!out_h) {
                   fclose(out); // LECAH (2019.06.24) : Fixing a vulnerability
			return 1;
             }
	}
	fprintf(out, "#\n"
	...
	if (out_h) fprintf(out_h, "#ifndef TESS_CONFIG_H\n#define TESS_CONFIG_H\n");
	if (out_h) fprintf(out_h, "\n/*\n" " * Automatically generated : don't edit\n" " */\n");

	if (out  ) fprintf(out,   "\nTESSWORKDIR=\"%s\"\n", get_current_dir_name());
	if (out_h) fprintf(out_h, "\n#define TESSWORKDIR \"%s\"\n", get_current_dir_name());
...
next:
	...
	fclose(out);
	if (out_h) {
		fprintf(out_h, "#endif /* TESS_CONFIG_H */\n");
		fclose(out_h);
        if (access("./include",0 ) < 0) 
            mkdir("include",0775);
		rename(".tmpconfig.h", "include/TSConfig.h");
	}
