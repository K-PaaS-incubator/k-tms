 ...
 * @author l k h  
 * @since 2017-04-17
 * @description 상관분석 > 출발 포트 팝업 컨텐츠, item view 
 */
define(function(require) {
	...
	var Tpl 							= require('text!tpl/popup/applicationSrcPortPopup.html'),			// port 팝업 조회 상단부분
		ItemTpl 						= require('text!tpl/popup/applicationSrcPortPopupItem.html'); 		// port 팝업 table list 부분
	
	var PortListItemView = Backbone.View.extend({
		tagName: 'tr',
		template: _.template(ItemTpl),
		events: {
			"click #srcIp"			: "popupSourceIp",		// 추적 팝업을 띄우기 위해 table Item 클릭
		...
		render: function() {
			this.$el.html(this.template({
				...
				data: {
					...
					//nDestinationPort
					nSourcePort: dataExpression.getPortFormat(this.model.get('nSourcePort')),
	...
	return Backbone.View.extend({
		...
		getPortPopupList: function(paging) {
			...
			thisView.collection.fetch({
				method: 'POST',
				url: 'api/selectApplicationSrcPortPopupList',
