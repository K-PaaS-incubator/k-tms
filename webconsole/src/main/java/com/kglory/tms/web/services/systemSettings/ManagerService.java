...
import com.kglory.tms.web.model.dto.*;
...
public class ManagerService {
    ...
    public ManagerVO getDbPassword() throws BaseException {
        ...
        result.setStrDbuid(jdbcUser);
        return result;
    }

    public void modifyDbPassword(ManagerDto dto, HttpSession session) throws BaseException {
        try {
            TableColumnsDto tableColumnsDto = new TableColumnsDto();

            // DB 패스워드 변경 쿼리
            tableColumnsDto.setUserId(jdbcUser);
            tableColumnsDto.setUserPwd(dto.getStrDbpwdInput());
            commonMapper.modifyDbPassword(tableColumnsDto);

            // db.properties 복호화 - 파일생성
            SystemUtil.fileSecurityDecryptCommand("/usr/local/mysql/data/db.properties", "/usr/local/mysql/data/db_web.properties");

            // db.properties 파일 내용 변경 코드
            String propertiesPath = "/usr/local/mysql/data/db_web.properties";
                // 내부 개발 전용
//            String propertiesPath = "D:\\workspace\\TAS CLOUD V1.0\\src\\main\\resources\\conf\\db.properties";
            String containWord = "password";
            ...
            while ((text = bufferedReader.readLine()) != null) {
                if (text.contains(containWord)) {
                    text = "password="+dto.getStrDbpwdInput();
                }
                textList.add(text);
            }

            BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter(propertiesPath));
            ...
            // db.properties 암호화
            SystemUtil.fileSecurityCommand("/usr/local/mysql/data/db_web.properties", "/usr/local/mysql/data/db.properties");

            // 복호화된 파일 삭제
            systemConfSvc.deleteDecryptedFile();

            // 패스워드 변경 성공 감사로그
            auditLogSvc.insertAuditLog(Constants.AUDIT_LTYPE1.ACTION.getValue(), Constants.SYSTEM_DB_MANAGE_MOD_SUCCESS, (String) session.getAttribute("Username"));
        } catch (Exception e) {
            logger.error(e.getLocalizedMessage(), e);
            // 패스워드 변경 실패 감사로그
            auditLogSvc.insertAuditLog(Constants.AUDIT_LTYPE1.ERROR.getValue(), Constants.SYSTEM_DB_MANAGE_MOD_FAIL, (String) session.getAttribute("Username"));
...
//            insertLIndex = dto.getlIndex();
    ...
    public void execNowDbBackup(ImDbBackupDto dto, HttpSession session) throws BaseException{
        ...
        if (!dayCheckVal.equals("00000")) {
            ...
            logger.debug("execNowDbBackup - policy");
...
//            Thread.sleep(1000 * 20);
    ...
    private void execDbBackupCommnad(String table, String file) throws BaseException{
        ...
        list.add("/bin/sh");
        list.add("-c");
        list.add("/usr/bin/mysqldump -u root -h 127.0.0.1 TESS " + table + " > " + file);
        SystemUtil.execCommand(list);
    }
