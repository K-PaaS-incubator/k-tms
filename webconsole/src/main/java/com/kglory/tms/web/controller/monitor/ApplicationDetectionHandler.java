...
import com.kglory.tms.web.services.monitor.ApplicationDetectionService;
import com.kglory.tms.web.util.DateTimeUtil;
import com.kglory.tms.web.util.TimeUtil;

public class ApplicationDetectionHandler extends TextWebSocketHandler {

    private static Logger logger = LoggerFactory.getLogger(ApplicationDetectionHandler.class);

    @Autowired
    ApplicationDetectionService applicationDetectionService;
    @Autowired
    ...
    public void handleMessage(WebSocketSession session, WebSocketMessage<?> message) throws BaseException {
        try {
            ...
            ApplicationMonitorSearchDto dto = objectMapper.readValue(payloadMessage, ApplicationMonitorSearchDto.class);
            ...
            while (session.isOpen()) {
                ...
                List<ApplicationDetectionEventVO> eventVOs = applicationDetectionService.selectApplicationDetectionEvent(dto);
                if (eventVOs.size() > 0) {
