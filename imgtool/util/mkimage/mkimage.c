...
extern uLong crc32 (uLong crc, const char *buf, unsigned int len);
...
NXTARG:	//	;
	...
	if (xflag) {
		if (ep != addr + sizeof(image_header_t)) {
			fprintf (stderr, "%s: For XIP, the entry point must be the load addr + %u\n",
				cmdname,
				(unsigned int)sizeof(image_header_t));
			exit (EXIT_FAILURE);
		}
	}
...
print_header (image_header_t *hdr)
{
	...
	printf ("Data Size:    %u Bytes = %.2f kB = %.2f MB\n",
	...
	if (hdr->ih_type == IH_TYPE_MULTI || hdr->ih_type == IH_TYPE_SCRIPT) {
		int i, ptrs;
		uint32_t pos;
		unsigned int *len_ptr = (unsigned int *)((unsigned char *)hdr + sizeof(image_header_t));
		...
		for (i=0; len_ptr[i]; ++i) {
			...
			printf ("   Image %d: %8u Bytes = %4d kB = %d MB\n",
