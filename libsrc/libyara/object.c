    ...
    int index)
{
  ...
  else if (index >= array->items->count)
  {
    ...
    for (int i = array->items->count; i < count; i++)
    ...
    ...)
{
  ...
  switch(field_obj->type)
  {
    ...
    case OBJECT_TYPE_INTEGER:
      return (unsigned)(((YR_OBJECT_INTEGER*) field_obj)->value) == UNDEFINED;
    default:
      break;
  }
    ...
    int indent)
{
  ...
  indent = min((size_t)indent, sizeof(indent_spaces));
  ...
  {
    case OBJECT_TYPE_INTEGER:
      if ((unsigned)(((YR_OBJECT_INTEGER*) object)->value) != UNDEFINED)
        printf(
            "%s%s = %u\n",
            indent_spaces,
            object->identifier,
