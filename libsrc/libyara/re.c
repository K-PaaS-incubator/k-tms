...
This modules implements a regular XXX expressions engine XXX based on Thompson's
...
/* yr_re_finalize
   모든 다른 쓰레드에서 이 모듈의 함수사용이 끝난 후 메인에서 불려질 것이다. */
...
/* yr_re_finalize_thread
   종료 전에, 현재 모듈에서 쓰고있는 모든 쓰레드들에 의해 불려질 것이다. */
