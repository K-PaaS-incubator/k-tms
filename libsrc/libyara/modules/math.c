...
define_function(data_entropy)
{
  ...
  if (offset < 0 || length < 0 ||  (size_t)offset < context->mem_block->base)
  {
  ...
  {
    if ((size_t)offset >= block->base &&
        (size_t)offset < block->base + block->size)
    {
      uint64_t data_offset = (uint64_t)offset - block->base;
      uint64_t data_len = min((uint64_t)length, block->size - data_offset);
	  unsigned int i;
    ...
    if (block->base + block->size > (size_t)(offset + length))
      break;
  }
...
define_function(data_mean)
{
  ...
  foreach_memory_block(context, block)
  {
    if ((size_t)offset >= block->base &&
        (size_t)offset < block->base + block->size)
    {
      uint64_t data_offset = (size_t)offset - block->base;
      uint64_t data_len = min((size_t)length, block->size - data_offset);
	  unsigned int i;
