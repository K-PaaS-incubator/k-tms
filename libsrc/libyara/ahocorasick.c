    ...
    const uint8_t input)
{
  ...
  if (state->depth <= MAX_TABLE_BASED_STATES_DEPTH)
  {
	  size_t offset;
// TODO TEST!!!!!!!!!!!!!!!!!!!!!! XXX
//  offset = offsetof(YR_AC_TABLE_BASED_STATE, transitions[input]);
    offset  = (size_t)&(((YR_AC_TABLE_BASED_STATE *)0)->transitions[input]);
      ...
      while (1)
      {
        ...
        if (temp_state != NULL)
        {
          ...
          else
          {
            ...
            // LECAH (2019.06.24) : Fixing a vulnerability (delete a redundant code)
            while (match->next != NULL) 
              match = match->next;
...
/* yr_ac_create_automaton
   새로운 오토마타를 만든다.*/
int yr_ac_create_automaton( YR_ARENA* arena, YR_AC_AUTOMATON** automaton)
{
  int result;
  YR_AC_STATE* root_state;
