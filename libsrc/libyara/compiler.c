...
/* compile 에러시 호출되는 콜백함수 등록 */
YR_API void yr_compiler_set_callback( YR_COMPILER* compiler, YR_COMPILER_CALLBACK_FUNC callback, void* user_data)
{
...
/* 컴파일러에 네임스페이스를 설정한다. */
int _yr_compiler_set_namespace( YR_COMPILER* compiler, const char* namespace_)
{
	YR_NAMESPACE* ns;
...
/*  컴파일러에 파일 추가?  */
YR_API int yr_compiler_add_file( YR_COMPILER* compiler, FILE* rules_file, const char* namespace_, const char* file_name)
{
	// yr_compiler_get_rules()가 호출된 이후 
	// yr_compiler_add_file()는 호출하면 안됀다.
